...
/***************************************************************************
 CopyCoordinatesTool
                                 A QGIS plugin
 Ferramenta para copiar coordenadas do mapa para o clipboard no CRS atual
...
class copy_coordenada(QgsMapTool):
    ...
    def canvasMoveEvent(self, event):
        ...
        if snap_match.isValid() and snap_match.hasVertex():
            ...
            # Calcula o tamanho do buffer em unidades do mapa com base em pixels
    ...
    def canvasPressEvent(self, event):
        ...
        # Pega as coordenadas do clique na tela
        screen_pos = event.pos()

        # Usa o snapping com as configurações atuais do projeto
        snap_match = self.snap_utils.snapToMap(screen_pos)

        # Se houver um ponto aderido, usa as coordenadas dele; caso contrário, usa o clique direto
        if snap_match.isValid() and snap_match.hasVertex():
        ...
        crs_name = map_crs.authid()  # Ex.: "EPSG:31983"

        # Formata as coordenadas como string (X, Y) no CRS atual
    ...
    def deactivate(self):
        """Limpa o rubber band quando a ferramenta é desativada."""
        self.rubber_band_outline.reset(QgsWkbTypes.PolygonGeometry)
        super().deactivate()

def copy_coordenada_def(iface):
    """Ativa a ferramenta para copiar coordenadas do mapa no CRS atual."""
...
def run(iface):
    """Função para executar a ferramenta de cópia de coordenadas no CRS atual."""
